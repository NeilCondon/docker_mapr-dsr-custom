...
#  upgrade                 upgrade Installer packages
#
#OPTIONS
...
DEPENDENCY_INSTALLER_DEB="$DEPENDENCY_BASE_DEB openssh-client openssh-server python-minimal sshpass sysv-rc-conf uuid-runtime"
DEPENDENCY_CLIENT_DEB="$DEPENDENCY_BASE_DEB apt-utils dnsutils file gnupg-curl \
...
DEPENDENCY_CLIENT_RPM="$DEPENDENCY_BASE_RPM initscripts syslinux openssl file"
DEPENDENCY_INSTALLER_RPM="$DEPENDENCY_BASE_RPM openssh-clients openssh-server python sshpass"
...
DEPENDENCY_INSTALLER_SUSE="$DEPENDENCY_BASE_SUSE openssh python"
...
JDK_UPGRADE_JRE=$NO
JDK_VER=0
export JVM_HEAP_DUMP_PATH="$MAPR_HOME/heapdumps"
...
cli_process() {
    if [ "$1" = "-h" ] || [ "$1" = "--help" ] || [ "$1" = "-help" ]; then
        echo "mapr-setup.sh cli <parameters to stanza>"
        exit 1
    fi
    ...
    success $YES
    local maprinstallercli="$MAPR_INSTALLER_DIR/bin/mapr-installer-cli"
    [ ! -f "$maprinstallercli" ] && msg_err "Installer CLI not found"
    msg "Invoking the MapR installer client..."
    sudo -u $MAPR_USER $maprinstallercli "$@"
    success
}
...
create_user() {
    ...
    if [ -z "$CREATE_USER_HOME_DI=" ]; then
        chown $MAPR_UID:$MAPR_GID "$HOME_BASE/$MAPR_USER"
    fi

    passwd $MAPR_USER > /tmp/pwresult.$$ 2>&1 << EOM
$MAPR_PASSWORD
$MAPR_PASSWORD
EOM
    if [ $? -ne 0 ]; then
        msg_err "Unable to set password for user $MAPR_USER\n - output is: $(cat /tmp/pwresult.$$)"
    fi
    rm -f /tmp/pwresult.$$
...
fetch_dependencies() {
    ...
    suse)
        ...
        if ! which sshpass > /dev/null 2>&1; then
            zypper --non-interactive -q --no-gpg-checks -p http://download.opensuse.org/distribution/leap/42.3/repo/oss/suse install -y sshpass
            if [ ! $? -eq 0 ]; then
              msg_err "Failed to install sshpass from Leap 42.3 repo. You have to manually install package"
              exit 1
            elif [ $? -eq 2 ]; then
              # Zypper provide speacial exit codes for different errors. 2 is for wrong arguments, for ex. - wrong URL.
              msg_err "Currently Leap 42.3 repo unreacheable. Check connection. You have to manually install package"
              exit 1
            fi
        fi
        zypper --non-interactive -q install -y ca-certificates
        ...
        if [ "$1" = "server" ]; then
            ...
            case "$pkgs" in
                *gettext*|*sysstat*)
                     ...
                     zypper --non-interactive -q install -y --force-resolution gettext
                ;;
            esac
            [ $OSVER_MAJ -ge 12 ] && pkgs="$pkgs python-pycurl" || pkgs="$pkgs python-curl"
        fi
        if zypper --non-interactive -q install -y -n $pkgs; then
...
fetch_installer_suse() {
    ...
    if [ -n "$MAPR_INSTALLER_PACKAGES" ]; then
        zypper --non-interactive -q --no-gpg-checks install -y -n $MAPR_INSTALLER_PACKAGES
    elif [ $ISCONNECTED -eq $YES ]; then
        # Create the mapr-installer repository information file
        ...
        zypper --non-interactive -q install -y -n mapr-installer-definitions mapr-installer
    else
        (cd "$HTTPD_REPO/installer/suse"; zypper --non-interactive -q install -y -n ./mapr-installer*)
    fi
...
fetch_jdk() {
    ...
    suse)
        ...
        zypper --non-interactive -q install -y -n $OPENJDK
        ;;
    ubuntu)
        if [ $JDK_UPDATE_ONLY -eq $YES ]; then
...
fetch_web_server() {
    ...
    suse) zypper --non-interactive -q install -y -n $HTTPD ;;
...
package_install_suse() {
    zypper --non-interactive install -y -n"$@" || \
...
repo_add_redhat() {
    [ -n "$http_proxy" ] && local options="--httpproxy $http_proxy"
    repo_add_key rpm $options --import
    cat > $1 << EOM
[MapR_$3]
...
container_install_thin() {
    ...
    url="$url/$file"
    file="/tmp/$file"
    msg "thin client download url is: $url"
...
RUN zypper --non-interactive up && zypper --non-interactive install -y -n $DEPENDENCY_INIT && zypper clean -a
EOM
}
...
installer_properties_update() {
    ...
    a["installer_admin_group"] = "'$MAPR_GROUP'" if "installer_admin_group" not in a else a["installer_admin_group"];\
    a["installer_admin_id"] = "'$MAPR_USER'" if "installer_admin_id" not in a else a["installer_admin_id"];\
    a["log_level"] = "INFO" if "log_level" not in a else a["log_level"];\
    a["log_rotate_cnt"] = int(5) if "log_rotate_cnt" not in a else a["log_rotate_cnt"];\
    a["os_version"] = "'${OS}_${OSVER}'" if "os_version" not in a else a["os_version"];\
    a["no_internet"] = inet if "no_internet" not in a else a["no_internet"];\
    a["container"] = cont if "container" not in a else a["container"];\'
...
upgrade_installer_suse() {
    zypper --non-interactive -q install -y \
...
if ( [ -n "$http_proxy" ] && [ "${http_proxy/http:}" = $http_proxy ]) || ([ -n "$https_proxy" ] && [ "${https_proxy/https:}" = "$https_proxy" ]); then
    msg_err "http_proxy or https_proxy environment variables are set but do not contain http[s]:// prefix - fix and retry"
fi
MAIN_CMD=$1 && shift
case "$MAIN_CMD" in
  cli) cli_process "$@" ;;
  ...
  ""|install)
      ...
      if [ $# -gt 0 ]; then
          msg_warn "install command takes no arguments" && usage
      fi
  ...
  reload)
      if [ $# -gt 0 ]; then
          msg_warn "reload command takes no arguments" && usage
      fi
  ...
  remove)
      if [ $# -gt 0 ]; then
          msg_warn "remove command takes no arguments" && usage
      fi
      installer_remove
      ;;
  update)
      if [ $# -gt 0 ]; then
          msg_warn "update command takes no arguments" && usage
      fi
  ...
  upgrade)
      if [ $# -gt 0 ]; then
          msg_warn "upgrade command takes no arguments" && usage
      fi
      check_os
      check_python_executable
